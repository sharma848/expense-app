...
import TransactionsModel from "../schema/transactions";

const router = express.Router();

router.get("/getAllByUserId", async (req: any, res) => {
    try {
        ...
        return res.status(404).json({ message: "No transaction record found for user." })
    } catch(err) {
...
router.put("/update/:transactionId", async (req, res) => {
    try {
        ...
        const record = await TransactionsModel.findByIdAndUpdate(transactionId, body);
